       ...
       with tf.Session() as sess:
           ...
           for xml_file in os.listdir(ann_dir):
               ...
               try :
                    ...
                    print('Bounding Boxes converted to numpy')
                    
                    sess.run(Rpn.Update_Params, feed_dict = {image_pl: Image, gt_boxes_pl: gt_box_np}) 
                    count += 1
                    print('session successful')
                    if count % 100 == 0:
                        Saver.save(sess, '/home/ise/RPN_rohit/Save_Data/RPN_weights', global_step=count)
                        print(' Training Image '+str(count)+' Finished')
                        print('DATA SAVED')
                    print('------Trained successfully on an image--------')
                except:
                  anti_count += 1  
                  print('Loading Error :-( ' +str(anti_count))
