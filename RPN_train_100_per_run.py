...
class RPN:
    ...
    def Feature_Map(self):                   
        ...
        self.im_shape = tf.cast(tf.shape(self.image), dtype=tf.float32)
        ...
        with tf.name_scope('conv1_1') as scope:
            kernel = tf.Variable(tf.truncated_normal([3, 3, 3, 64], dtype=tf.float32,
                                                     stddev=1e-2), name='weights', trainable=False)
        ...
        with tf.name_scope('conv5_3') as scope:
            ...
            biases = tf.Variable(tf.constant(0.0, shape=[512], dtype=tf.float32),
                                 trainable=True, name='biases')
            out = tf.nn.bias_add(conv, biases)
            self.f_map = tf.nn.relu(out, name=scope)
            self.parameters += [kernel, biases]
                    
        self.f_map_shape = tf.cast(tf.shape(self.f_map), dtype=tf.float32)
    ...
    def Cls_Reg(self):
        ...
        with tf.name_scope('rpn_f_vector') as scope:
            kernel = tf.Variable(tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32,
                                                     stddev=1e-2, name='weights'), trainable=True)
            conv = tf.nn.conv2d(self.f_map, kernel, [1, 1, 1, 1], padding='SAME')         #WHAT ABOUT EDGES OF FEATURE MAP????
        ...
        # Reshaping & Storing results in prediction_dict
...
if __name__ == '__main__':         
    ...
    def Train():
       ...
       ''' Train on 100 images first 
           Save weights
           Train on next 100 images'''
       ...
       #Session
       with tf.Session() as sess:
           # Assigning weights and initializing variables
           sess.run(tf.global_variables_initializer())
           sess.run(Rpn.assign_wts)
           # Saving Graph
           writer = tf.summary.FileWriter("RPN_1_by_1_graph", sess.graph)
           ...
           count = 0
           anti_count = 0
           # Modify start and for each run of program
           i = 0
           start = 0
           end = 50
                   ...
                   try :
                        ...
                        for sroot in root:
                            ...
                            if sroot.tag == 'filename':
                                ...
                                Image    = cv2.imread(img_dir, 1)
                            # Getting scaled image with 600 
                            # as length of the smaller side                                            
                            if sroot.tag == 'size':
                                ...
                                Image = cv2.resize(Image,(int(width),int(height)))                           
                            # Getting bounding-boxes co-ordinates                           
                            if sroot.tag == 'object':
                                for ssroot in sroot:
                        ...
                        count += 1
                        print('Trained successfully on image '+str(count))
                   except:
                        anti_count += 1
                        pass
               else:
           ...
           print('Perfect count :'+str(count))
           print('Error   count :'+str(anti_count))
           Saver.save(sess, 'Saved Weights', global_step=count)      
